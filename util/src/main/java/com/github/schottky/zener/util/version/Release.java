...
public enum Release implements Version {
    ...
    public int compareToVersion(@NotNull Version version) {
        ...
        } else if (version.getClass() == Snapshot.class) {
            Snapshot snapshot = (Snapshot) version;
            int result = this.compareToVersion(snapshot.asRelease());
    ...
    public @NotNull Snapshot snapshot(int year, int week, String identifier) {
        return new Snapshot(this, year, week, identifier);
    }

    @Contract(value = "_ -> new", pure = true)
    public @NotNull PreRelease preRelease(int preReleaseNumber) {
        return new PreRelease(this, preReleaseNumber);
    }

    @Contract(value = "_ -> new", pure = true)
    public @NotNull ReleaseCandidate releaseCandidate(int releaseCandidateNumber) {
        return new ReleaseCandidate(this, releaseCandidateNumber);
    }
